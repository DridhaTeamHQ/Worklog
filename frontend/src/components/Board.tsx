        ...
        return (
          <section
            ...
            className={`flex w-72 shrink-0 flex-col rounded-xl border p-3 transition-colors ${
              active ? 'border-brand-400 bg-brand-50' : 'border-ink-200 bg-ink-50/60'
            }`}
          >
            <header className="mb-3 flex items-center gap-2 px-1">
              <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${column.dot}`} aria-hidden />
              <h3 className="min-w-0 truncate text-sm font-bold text-ink-900">{column.label}</h3>
                ...
                column.items.map((item) => {
                  ...
                  return (
                    <article
                      ...
                      className={`relative cursor-grab rounded-lg border border-ink-200 bg-white p-3 shadow-sm transition-shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 ${
