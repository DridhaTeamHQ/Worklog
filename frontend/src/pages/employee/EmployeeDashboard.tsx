...
export function EmployeeDashboard() {
  ...
  return <div className="space-y-7">
    <div className="flex items-end justify-between gap-4"><div><p className="text-sm font-medium text-primary-strong">{formatDate(new Date().toISOString())}</p><h1 className="display-title mt-1 text-3xl text-foreground sm:text-4xl">{getGreeting()}, {user?.name.split(' ')[0]}</h1><p className="mt-2 text-sm text-muted-foreground">Here’s your work at a glance.</p></div><Link to="/employee/tasks-assigned" className="btn-secondary hidden sm:inline-flex">View all tasks <ArrowRight className="h-4 w-4" /></Link></div>
